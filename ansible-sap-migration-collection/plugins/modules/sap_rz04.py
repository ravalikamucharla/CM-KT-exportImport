...
def main():
    try:
        ...
        for i, temp_dict_t in enumerate(targetvalues_list):
            if temp_dict_t["Opera mode type"] != "T":
                temp_dict = copy.deepcopy(temp_dict_t)
                temp_dict.update({"ACTION":"D"})
                new_target_list1.append(temp_dict)
            else:
                op_mode_name.append(temp_dict_t["Operation mode"])


        for i, temp_dict_s in enumerate(sourcevalues_list):
            if temp_dict_s["Opera mode type"] != "T":
                temp_dict_s.update({"ACTION":"I"})
                new_target_list1.append(temp_dict_s)
        ...
        new_target_list2 = []
        for i, temp_dict_t in enumerate(targetvalues_list):
            if temp_dict_t["Operation mode"] not in op_mode_name:
                temp_dict = copy.deepcopy(temp_dict_t)
                temp_dict.update({"ACTION":"D"})
                new_target_list2.append(temp_dict)
        ...
        new_target_list3 = []
        for _, temp_dict_t in enumerate(targetvalues_list):
            temp_dict = copy.deepcopy(temp_dict_t)
            temp_dict.update({"ACTION":"D"})
            new_target_list3.append(temp_dict)
            
        for _, temp_dict_s in enumerate(sourcevalues_list):
            temp_dict = copy.deepcopy(temp_dict_s)
            temp_dict.update({"ACTION":"I"})
            new_target_list3.append(temp_dict)


        targetvalues_list2 = new_target_list1 + [""] + new_target_list2 + [""] + new_target_list3
        ...
        #if data_found == True:
        column_found = True
        for i, ele in enumerate(targetvalues_list2):
            if ele == "":
                abap_input.append("")
                column_found = True
            else:
                if column_found:
                    temp_column = ""
                    for key, _ in ele.items():
                        temp_column = temp_column + "|" + key
                    abap_input.append(temp_column[1:])
                    column_found = False

                    temp_string = ""
                    for _, value in ele.items():
                        if value == "NULL":
